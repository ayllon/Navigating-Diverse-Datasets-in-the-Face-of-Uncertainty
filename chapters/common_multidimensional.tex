...
% TODO: Bring what is missing from the arxiv paper

\section{Introduction}

Nowadays, it is not uncommon for many types of users ---from proficient
data scientists to enthusiasts without formal training--- to dive
into overwhelming sets of data looking for any relevant pattern they
can find. This data may consist of raw files that have not yet been ingested
into a database system and for which the schema may be unfamiliar and not
adequately documented. Furthermore, the entire data set may be composed
of multiple files with heterogeneous schemes~\cite{alawini2016,zhang2015astronomy}.

For the \emph{in-situ} interactive exploration, there are many proposals at different
levels: database (indexes, physical layout),
middleware (pre-fetching, query approximation) and user interface (visualization,
assisted exploration) \cite{Idreos2015}. For more details, including a survey of existing
solutions, we refer the reader to an exhaustive systematic mapping of the literature
previously published~\cite{Alvarez2019}.
As a result of this survey, we realized that most of the solutions
treat files separately, leaving it to the end-user to work out how they are related.
This is an observation shared by other authors~\cite{Silva2016}.

Therefore, our goal is to assist users to understand how multiple raw files
are related; to identify shared sets of attributes, and to facilitate
...
Foreign key discovery is outside the scope of this paper.
...
% This property is similar to the one proposed for INDs\cite{DeMarchi2002}, with the exception that even if $d \models I_2$, there is a probability to falsely reject $I_1$ bound by the significance level $\alpha$.
...
However, this is not always true for the EDD finding problem. The statistical test
