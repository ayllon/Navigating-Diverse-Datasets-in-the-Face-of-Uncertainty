...
Given the fast pace at which the field moves, older papers have probably been
...
For the 2017 survey, all searches were done on May 16th, 2017, and they yielded a total of \numprint{5 525} articles.
...
Due to limitations of the search engine, only \numprint{2000} articles were found, later reduced to 
\numprint{1884} after de-duplication\footnotemark. Forward snowballing provided \numprint{584} articles, 
of which \numprint{452} are articles referencing \textsc{BlinkDB}~\cite{Agarwal2013}!
From this total of \numprint{2 468} articles, \numprint{89} --- 3.61\% --- were accepted.

\footnotetext{
    De-duplication of \emph{the same} article appearing multiple times on the search results.
}
...
While there are two main forums, summing \numprint[\%]{40.18} of all the publications,
...
\begin{table}[hptb]
  ...
  \begin{tabularx}{\textwidth}{X r r} \hline
    ...
    \textbf{Database Layer}                     & \textbf{108} & \textbf{27} \\
      ...
      %\hspace{0.5em} Spatial Query              &  6 & 0 \\
...
The vast majority of papers considered by this study---\numprint[\%]{83.69}---fall within the
\emph{proposal of solution} research type.

Meanwhile, \emph{evaluation} and \emph{validation} research are represented
just by a \numprint[\%]{7.85} and \numprint[\%]{5.14}, respectively.
Only 37 documents (\numprint[\%]{11.2}) include some sort of user study:
...
\textbf{Resource-aware adaptive indexing for in situ visual exploration and
...
\textbf{\textsc{AccuracyTrader}}~\cite{Han2016} is a distributed approximate processing 
